...
if (isset($reservering)) {
    ...
    </p>Hartelijk dank voor je bestelling van concertkaartjes. De volgende gegevens zijn geregistreerd:\n\n</p>
    <ul>
        <li>Naam: {$naam}</li>\n\n
...
<head>
    ...
    <link rel="icon" type="image/png" href=<?php echo $favicon; ?>> ======= if
    ($reservering['aantal_red'] > 0) $message .= "<li>{$kaartjes_red}
        {$concert['euro_red']}</li>\n"; if ($reservering['aantal_kind'] > 0)
    $message .= "<li>$kaartjes_kind {$concert['euro_kind']}</li>\n"; if
    (isset($reservering['flyers']) & $reservering['flyers'] == 1) { $message .=
    "<li>Je hebt aangegeven dat je op de hoogte gehouden wilt worden van
        toekomstige concerten. We zullen je hierover van tijd tot tijd emails
        sturen.</li>\n"; } $message .= <<<MESSAGE </ul>
        <li>\nHet totale verschuldigde bedrag is {$reservering['euro_totaal']}.
            Dit bedrag heb je reeds betaald via iDeal. In je email ontvang je
            een concertkaartje. Neem dit SVP mee naar het concert, geprint op
            papier of op je telefoon. Op dat kaartje staat een QR code. Bij het
            concert wordt die gescand.</li>
        </ul> MESSAGE; if (isset($reservering['opmerkingen']) &
        $reservering['opmerkingen'] != "") { $reservering['opmerkingen'] =
        htmlentities(stripslashes($reservering['opmerkingen'])); $message .= "
        <p>Je opmerkingen waren: {$reservering['opmerkingen']}</p>\n"; }
        $message .= "<p>Met muzikale groet,\n\n<br><br>{$organisator}</p>
        \n<br>"; d($message); } else { if (isset($logo_url) AND $logo_url != '')
        {$logo=$url.rawurlencode($logo_url); $message = <<<MESSAGE <header
            class="w3-panel-0"><img src="{$logo}" alt="logo"
                style="width: 100%; max-width: 600px; height: auto; border: none;">
            </header>\n MESSAGE; } else $message = ''; $message .= <<<MESSAGE
                <p>Beste {$reservering['voornaam']},</p>\n </p>Hartelijk dank
                voor je bestelling van concertkaartjes. Helaas is de betaling
                niet gelukt. De kaartjes zijn dus nog niet gereserveerd. We
                raden je aan om de bestelling en betaling nogmaals uit te
                voeren. <a href="javascript:history.go(-3)">Klik op deze link om
                    terug te gaan naar je bestelling</a>.</p> MESSAGE; $message
                .= "<p>Met muzikale groet,\n\n<br><br>{$organisator}</p>\n<br>";
